...
function TheedManager:spawnSceneObjects(currentPhase)
	...
	for i = 1, #objectTable, 1 do
        ...
        playClientEffectLoc(pObject, "clienteffect/holoemote_imperial.cef", "naboo", SceneObject(pObject):getPositionX(), SceneObject(pObject):getPositionZ(), SceneObject(pObject):getPositionY(), SceneObject(pObject):getParentID())
        if (pObject ~= nil) then
...
function TheedManager:spawnMobilesPhase2()
    if (readData("theedphase2:npc:object:") == 0) then
        ...
        writeData("theedphase2:npc:object:" .. 1, mobileID)
	end
    return 0
end

function TheedManager:spawnMobilesPhase2_2(pRebel_Extreme2, pKiller)
	if (readData("theedphase2_2:npc:object:") == 0) then
...
function TheedManager:despawnMobilesPhases(currentPhase)
    if (currentPhase == 1) then
        local objectID = readData("theedphase:npc:object:" .. 1)
		local pMobile = getSceneObject(objectID)
    ...
    elseif (currentPhase == 2) then
        -- 2_1
        local objectID = readData("theedphase2:npc:object:" .. 1)
		local pMobile = getSceneObject(objectID)
        -- 2_2
